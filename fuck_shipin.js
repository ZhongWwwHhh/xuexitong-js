...
//更改这一处！！！！！
var xuanzezhangjie = 762413332;

function main() {
    try { var frameObj = $("iframe").eq(0).contents().find("iframe.ans-insertvideo-online"); }
    catch (err) { console.error("无内容或无法播放，下一节"); nextUnit(); return; }
...
function nextUnit() {
    ...
    xuanzezhangjie += 1;
    setTimeout(() => {
        getTeacherAjax('235970252', '79896377', xuanzezhangjie.toString());
        console.log("%c行了别看了，我知道你学会了，下一节", "color:red;font-size:18px");// (已经跳转" +(++window.unitCount)+"次)");
        if (1 > 0) { setTimeout(() => main(), 10000) }
    }, 6000);
}
